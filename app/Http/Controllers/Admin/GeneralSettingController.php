...
class GeneralSettingController extends Controller
{
    ...
    {
        $fileSizeLimit = substr(ini_get('upload_max_filesize'), 0, -1);
        $request->validate([
            'site_name'            => 'required|string|max:40',
            'base_color'           => 'nullable|regex:/^[a-f0-9]{6}$/i',
            'secondary_color'      => 'nullable|regex:/^[a-f0-9]{6}$/i',
            'timezone'             => 'required|integer',
            'attachment_file_type' => 'required',
            'file_size_limit'      => ['required', 'numeric', 'max:' . $fileSizeLimit]
        ]);
        ...
        $general->attachment_file_type = $request->attachment_file_type;
        $general->file_size_limit = $request->file_size_limit;
        $general->save();
