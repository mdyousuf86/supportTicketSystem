                                ...
                                <div class="form-group">
                                    <select class="form--control form-select" name="ticket_status_id">
                                        @foreach ($statuses as $status)
                                            <option value="{{ $status->id }}" @selected($status->id == 2)>
                                                {{ __($status->title) }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="image-upload">
                                    <div class="form-group mb-0">
                                        <label for="inputAttachments">@lang('Attachments')</label> <span
                                            class="image-text text--danger">@lang('Max 5 files can be uploaded. Maximum upload size is'){{ ini_get('upload_max_filesize') }}</span>
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <form method="post" action="{{ route('admin.ticket.close', $ticket->ticket_number) }}">
                        @csrf
                        <input type="hidden" name="replayTicket" value="4">
